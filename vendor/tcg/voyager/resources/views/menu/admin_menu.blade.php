...
@foreach ($items->sortBy('order') as $item)
    ...
    @php
        ...
        if(!$originalItem->children->isEmpty())
        {
            ...
            $linkAttributes =  'href="#' . str_slug($item->title, '-') .'-dropdown-element-'.($item->id).'" data-toggle="collapse" aria-expanded="'. (in_array('active', $listItemClass) ? 'true' : 'false').'"';
    ...
    <li class="{{ implode(" ", $listItemClass) }}">
        ...
        @if(!$originalItem->children->isEmpty())
        <div id="{{ str_slug($originalItem->title, '-') }}-dropdown-element-{{ str_slug($originalItem->id) }}" class="panel-collapse collapse {{ (in_array('active', $listItemClass) ? 'in' : '') }}">
